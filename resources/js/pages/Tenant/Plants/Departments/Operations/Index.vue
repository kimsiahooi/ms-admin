...
import { Department, Status, StatusLabel } from '@/types/Tenant/plants/departments';
...
const props = defineProps<{
    ...
    operations: PaginateData<Operation[]>;
    options: {
        select: {
            statuses: SelectOption<Operation['status']['value']>[];
        };
        switch: {
...
const search = () =>
    router.visit(
        route('plants.departments.operations.index', {
            ...pickBy(filter.data()),
            tenant: tenant?.id || '',
        ...
        cell: ({ row }) => {
            ...
            return h('div', { class: 'flex items-center gap-2' }, [
                h(StatusSwitch, {
                    ...
                    method: 'put',
                    href: route('plants.departments.operations.toggleStatus', {
                        tenant: tenant?.id || '',
                        plant: props.plant.id,
                        department: props.department.id,
                ...
                h(
                    DeleteDialog,
                    {
                        ...
                        route: route('plants.departments.operations.destroy', {
                            tenant: tenant?.id || '',
                            plant: props.plant.id,
                            department: props.department.id,
                            operation: operation.id,
...
const columnVisibility: VisibilityState<Operation> = {
    id: false,
    description: false,
};
...
const submit = () =>
    form.post(route('plants.departments.operations.store', { tenant: tenant?.id || '', plant: props.plant.id, department: props.department.id }), {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            form.reset();
