...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    {
        title: 'Edit',
        href: route('plants.departments.edit', { tenant: tenant?.id || '', plant: props.plant.id, department: props.department.id }),
    },
]);
...
const form = useForm({
    ...
    status: props.department.status.switch ?? undefined,
});

const submit = () =>
    form.put(route('plants.departments.update', { tenant: tenant?.id || '', plant: props.plant.id, department: props.department.id }), {
        preserveScroll: true,
        preserveState: true,
    });
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <Layout>
            <form @submit.prevent="submit">
                <Card :title="`Edit ${department.name}`">
