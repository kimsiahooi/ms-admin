...
const search = () => router.visit(route('plants.index', { ...pickBy(filter), tenant: tenant?.id || '' }));
...
const defaultStatus = computed<Plant['status']>(() => props.options.statuses.find((status) => status.is_default)?.value ?? 'ACTIVE');
