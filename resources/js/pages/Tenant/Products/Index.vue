...
const props = defineProps<{
    ...
    options: {
        select: {
            statuses: SelectOption<Product['status']['value']>[];
        };
        switch: {
...
const search = () =>
    router.visit(route('products.index', { ...pickBy(filter.data()), tenant: tenant?.id || '' }), {
        preserveScroll: true,
        preserveState: true,
    });
const reset = () => {
...
const columnVisibility: VisibilityState<Product> = {
    id: false,
    description: false,
};
